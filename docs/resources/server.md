...
The `product_name`, `region_name`, and OS (`os_name`/`os_dist`) inputs are resolved against the live Gigahost catalog during `terraform apply`, so an unknown or unavailable value surfaces as an apply-time error rather than during `terraform plan`.
